...
class battleships_graph:
    ...
    def create_clusters(self, rel_ids, min_val, max_val, kmeans_seeds_num=3):
        ...
        k = self.find_optimal_k(rel_ids, rel_poolers, min_val, max_val)
    ...
    def find_optimal_k(self, rel_ids, rel_poolers, min_val, max_val):
        ...
        with multiprocessing.Pool(processes=int(multiprocessing.cpu_count() / 3) - 1) as pool:
            scores = list(pool.starmap(self.calc_k_means, zip(k_list,
                                                              ...
                                                              repeat(min_val),
                                                              repeat(max_val))))
        sse_vals = [sse[1] for sse in scores]
        sil_vals = [sse[2] for sse in scores]
        kn = KneeLocator(k_list, sse_vals, curve='convex', direction='decreasing')
        if kn.knee is not None:
            return kn.knee
        else:
            return k_list[np.argmax(sil_vals)]
    ...
    def add_automatic_edges(self, pooler_id, edges_set, automatic_edges_num, neighbors, bucket2orig):
        ...
        for neighbor in neighbors[0][1:]:
            if added >= automatic_edges_num:
                break
            if min(bucket2orig[neighbor], bucket2orig[pooler_id]) < self.available_pool_size:
    ...
    def process_candidates(self, candidate_neighbors_dict, edges_ratio, edges_set):
        ...
        for pair in candidate_neighbors_dict.keys():
            if counter > edges_limit:
                break
            elif min(pair[0], pair[1]) < self.available_pool_size:
                edges_set.add((pair[0], pair[1]))
                counter += 1
        return edges_set

    def create_bucket_edges(self, bucket_ids, label_type, automatic_edges_num=5, edges_ratio=0.05):
        automatic_edges_num = automatic_edges_num if label_type < 2 else 2 * automatic_edges_num
